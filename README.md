...
Touch long-press detection (custom 
useLongPress
 hook) to surface admin controls on mobile without a separate UI affordance
Lazy image loading for performance
Graceful out-of-bounds protection when images are deleted while the modal is open
🎠 Hero Carousel
An animated landing carousel (
HeroCarousel
) auto-advances every 10 seconds with alternating slide-from-left / slide-from-right CSS transitions and interactive dot navigation.
...
Admins access a structured form (
CarpetForm
) to register a new carpet. Key implementation details:
...
Deployment	Vercel (with SPA fallback routing via 
vercel.json
)
Notable Engineering Decisions
Custom 
useLongPress
 hook (useRef-based timer management) shared across three components to eliminate duplicated touch-event logic; handles auto-dismiss, cleanup on unmount, and suppresses the OS context menu on image long-press.
Custom 
useCarpets
 hook wrapping TanStack Query with a discriminated-union 
SearchParams
 type, enabling exhaustive type-safe switching between all three search modes.
Custom 
useImageStorage
 hook encapsulating the entire upload + Firestore write lifecycle, returning live progress, urls, and error state to the consuming form.
Firestore Timestamp normalization — all Timestamp fields are converted to native JS 
Date
 at the data-layer boundary so UI components never handle Firestore-specific types directly.
...
structuralSharing: false on the TanStack Query client to prevent Firestore Timestamp/
Date
 objects from failing reference-equality diffing.
